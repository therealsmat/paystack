defmodule PaystackApiTest do
  ...
  describe "get/1" do
    test "makes request with the correct configuration" do
      ...
      |> expect(:get, fn url, headers ->
        ...
        assert Enum.sort(headers) ==
                 Enum.sort([{:Accept, "Application/json"}, {:Authorization, "Bearer 12345678"}])
    ...
    test "responds with correct paystack data" do
      ...
      |> expect(:get, fn _, _ ->
        success_response(
          201,
          ~s<{"status": true, "message": "Authorization URL created", "data": {"authorization_url": "https://checkout.paystack.com/0peioxfhpn", "access_code": "0peioxfhpn"}}>
        )
      end)
    ...
    test "fires appropriate telemetry event" do
      ref =
        assert_telemetry_is_executed(fn _measurements, meta ->
  ...
  describe "put/2" do
    test "makes request with the correct configuration" do
      ...
      @http
      |> expect(:put, fn url, body, headers ->
  ...
  describe "delete/2" do
    test "makes request with the correct configuration" do
      ...
      @http
      |> expect(:request, fn :delete, url, body, headers ->
      ...
      assert {:ok, _} =
               PaystackApi.delete("/dedicated_account/split", %{account_number: "0033322211"})
    end

    test "responds with correct paystack data" do
      @http
      |> expect(:request, fn _, _, _, _ ->
    ...
    test "responds gracefully when there is an unknown error" do
      @http
      |> expect(:request, fn :delete, _, _, _ -> failed_response("Destination unreachable") end)
